...
.PHONY: build test fmt lint policy check install clean dist verify-dist notarize fixture-app smoke-local perf-local coverage perf benchmark
...
fmt:
	swift format --recursive --in-place Sources Entry Tests Benchmarks scripts/ax-smoke-check.swift scripts/ax-perf-check.swift scripts/send-hotkeys.swift Package.swift

lint:
	swift format lint --recursive --strict Sources Entry Tests Benchmarks scripts/ax-smoke-check.swift scripts/ax-perf-check.swift scripts/send-hotkeys.swift Package.swift
